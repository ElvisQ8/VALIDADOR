import streamlit as st
import pandas as pd
import openpyxl
from io import BytesIO

def procesar_archivo(archivo_cargado, plantilla):
    # Cargar archivo de plantilla seleccionado
    plantilla_wb = openpyxl.load_workbook(plantilla)
    plantilla_ws = plantilla_wb["PECLD07792"]
    duplicado_ws = plantilla_wb["Duplicado"]
    standar_ws = plantilla_wb["STD (PECLSTDEN02)"]
    
    # Cargar archivo CUALQUIERA.xlsx
    wb = openpyxl.load_workbook(archivo_cargado, data_only=True)
    if "BD_densidad_2020" not in wb.sheetnames:
        st.error("El archivo cargado no contiene la hoja 'BD_densidad_2020'")
        return None, None
    ws = wb["BD_densidad_2020"]
    
    # Leer los datos desde A10 hasta R en la hoja BD_densidad_2020
    datos = []
    for row in ws.iter_rows(min_row=10, max_col=17, values_only=True):
        if any(row):  # Solo tomar filas con datos
            datos.append(row)
    
    # Pegar datos en la plantilla desde A28
    start_row = 28
    for i, row in enumerate(datos, start=start_row):
        for j, value in enumerate(row, start=1):
            plantilla_ws.cell(row=i, column=j, value=value)
    
    # Guardar cambios en un BytesIO para permitir la descarga
    output = BytesIO()
    plantilla_wb.save(output)
    output.seek(0)
    return output, plantilla_wb

st.title("Validador de registro de datos - densidad")

# Selección de plantilla
opciones_plantilla = {
    "ROSA LA PRIMOROSA": "PLANTILLA.xlsx",
    "MILAGROS CHAMPIRREINO": "PLANTILLA1.xlsx",
    "YONATAN CON Y": "PLANTILLA2.xlsx"
}

plantilla_seleccionada = st.selectbox("Seleccione el responsable:", list(opciones_plantilla.keys()))
plantilla_path = opciones_plantilla[plantilla_seleccionada]

# Subir archivo
archivo_cargado = st.file_uploader("Carga archivo de datos en Excel", type=["xlsx"])

if archivo_cargado is not None:
    output, plantilla_wb = procesar_archivo(archivo_cargado, plantilla_path)
    if output:
        st.download_button(label="Descargar archivo procesado", data=output, file_name="Certificado.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        
        # Generar y descargar archivos CSV
        plantilla_ws = plantilla_wb.active
        df = pd.DataFrame(plantilla_ws.values)
        df.columns = df.iloc[26]
        df = df[27:].reset_index(drop=True)
        
        nombre_base = plantilla_ws.cell(row=28, column=1).value
        responsable = plantilla_seleccionada
        
        archivo1 = df[~df["QC_Type"].isin(["DSTD", "DEND"])][["Holeid", "From", "To", "Sample number", "Displaced volume (g)", "Wet weight (g)", "Dry weight (g)", "Coated dry weight (g)", "Weight in water (g)", "Coated weight in water (g)", "Coat density", "moisture", "Determination method", "Date", "comments"]]
        archivo1.insert(14, "Responsible", responsable)
        
        archivo2 = df[df["QC_Type"] == "DEND"][["hole_number", "depth_from", "depth_to", "sample", "displaced_volume_g_D", "dry_weight_g_D", "coated_dry_weight_g_D", "weight_water_g", "coated_wght_water_g", "coat_density", "QC_type", "determination_method", "density_date", "comments"]]
        archivo2.insert(13, "Responsible", responsable)
        
        archivo3 = df[df["QC_Type"] == "DSTD"][["hole_number", "displaced_volume_g", "dry_weight_g", "coated_dry_weight_g", "weight_water_g", "coated_wght_water_g", "coat_density", "DSTD_id", "determination_method", "density_date", "comments"]]
        archivo3.insert(10, "Responsible", responsable)
        
        st.download_button("Descargar Archivo 1", data=archivo1.to_csv(index=False), file_name=f"{nombre_base}.csv", mime="text/csv")
        st.download_button("Descargar Archivo 2", data=archivo2.to_csv(index=False), file_name=f"{nombre_base}__QC-DUP.csv", mime="text/csv")
        st.download_button("Descargar Archivo 3", data=archivo3.to_csv(index=False), file_name=f"{nombre_base}__QC-STD.csv", mime="text/csv")
